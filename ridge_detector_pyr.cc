...
int main(int argc, char ** argv){
	...
	for(int i=1; i<argc; i++){
		...
		for(int j=0; j<pyr.size(); j++){
			...
			clusters.clear();
			cluster(ridgedetector, L2(cvPoint2D32f(0,0), cvPoint2D32f(pyr[j].width, pyr[j].height)), clusters);
			draw_clusters( &im3, clusters, pow(2.0, j), CV_RGB(0,j*255/pyr.size(),255) );
		}
		im3.show("res");
		cvWaitKey();
	}
