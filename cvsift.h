...
CV_INLINE CvScalePoint cvScalePoint( float x, float y, float level=0, int subLevel=0, float angle=0 ){
	CvScalePoint sp;
	sp.center.x = x; 
	sp.center.y = y;
	sp.angle = angle;
...
CvSeq * cvFindFeaturesDoG(  const CvArr * im, CvMemStorage * storage, CvSeq ** features, 
		float dogThresh=2.0, float edgeRatio=20.0, float contrastThresh=0.03, float sigma=1.6,    
